...
def train(data_dir,val_dir):
    ...
    dataloader_train = torch.utils.data.DataLoader(data, shuffle = True, batch_size=32, num_workers=4)
...
def val(data_dir,model):
    t = [transforms.Resize((299,299)),transforms.ToTensor(),
         transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])]
    transform = transforms.Compose(t)
    data = Data(data_dir,transform)   
